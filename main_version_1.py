...
class NeuralNetwork:
    def __init__(self, network_structure): #the network structure already contains the bias
        ...
        self.weights_by_spaces_betweens_layers = self.init_weights()
        self.epochs = None
    ...
    def __predict(self, activation):
        ...
        for right_side_space_layer_index in range(1, len(self._network_structure)): # w is matrix, x is vector -> product gives a vector
            ...
            w = self.weights_by_spaces_betweens_layers[left_side_space_layer_index]
    ...
    def _learn_by_mini_batch(self, mini_batch):
        nabla_w = [np.zeros(w.shape) for w in self.weights_by_spaces_betweens_layers]
        ...
        for x, y in mini_batch: #x is the array of data, y is the result
            ...
            self.weights_by_spaces_betweens_layers = [w - self.eta * nw
                                                      for w, nw in zip(self.weights_by_spaces_betweens_layers, nabla_w)]
    ...
    def _feed_forward_and_back_propagation(self, x, y):
        ...
        #feedforward
        for i, weights_of_all_neurons_in_current_layer in enumerate(self.weights_by_spaces_betweens_layers):
            ...
            if self._is_need_to_add_activation_to_vector(self.weights_by_spaces_betweens_layers, i):
        ...
        for i in range(2, self.number_of_layers):
            ...
            weights_by_current_layer = self.weights_by_spaces_betweens_layers[-i + 1]
